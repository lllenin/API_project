...
func NewStorage(connStr string) (*Storage, error) {
	...
	s := &Storage{
		...
		prepGetTaskByID:       `SELECT id, title, description, status, user_id FROM tasks WHERE id = $1`,
		prepGetTasks:          `SELECT id, title, description, status, user_id FROM tasks WHERE user_id = $1`,
		prepUpdateTask:        `UPDATE tasks SET title = $1, description = $2, status = $3 WHERE id = $4`,
		prepDeleteTask:        `DELETE FROM tasks WHERE id = $1`,
...
func (s *Storage) GetTasks(ctx context.Context, userID string) ([]models.Task, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&task.ID, &task.Title, &task.Description, &task.Status, &task.UserID); err != nil {
			log.Println("[ERROR] Ошибка при сканировании задачи:", err)
			return nil, err
		}
...
func (s *Storage) DeleteTask(ctx context.Context, id string) error {
	ctx, cancel := context.WithTimeout(ctx, 15*time.Second)
	defer cancel()
	stmt, err := s.conn.Prepare(ctx, "delete_task", s.prepDeleteTask)
	if err != nil {
		log.Println("[ERROR] Не удалось подготовить запрос на удаление задачи:", err)
		return err
	}
	ct, err := s.conn.Exec(ctx, stmt.Name, id)
	if err != nil {
		log.Println("[ERROR] Не удалось удалить задачу:", err)
		return err
	}
	if ct.RowsAffected() == 0 {
	...
	log.Println("[SUCCESS] Задача успешно удалена:", id)
	return nil
}
...
func (s *Storage) GetUserByUsername(username string) (*models.User, error) {
	...
	if err != nil {
		log.Println("[ERROR] Не удалось подготовить запрос на получение пользователя по username:", err)
		return nil, err
	}
